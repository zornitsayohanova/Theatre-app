    ...
    function Seatchart(options) {
        ...
        */
        ...
        var releaseSeat = function releaseSeat(id) {
            ...
            var type = seat.classList[1];
            console.log(type)
            seat.className = 'sc-seat available ' + type;
        };
        ...
        var updateTotal = function updateTotal() {
            if (cartTotal !== undefined) {
                cartTotal.textContent = 'Общо: {0} {1}'.format(self.getTotal().toFixed(2), options.cart.currency);
            }
        ...
        var createCartItem = function createCartItem(seat) {
            ...
            seatPrice.textContent = '{0} {1}'.format(seat.price.toFixed(2),options.cart.currency);
        ...
        var rowName = function rowName(index, disabled, disabledCount) {
            if (!disabled) {
                return (index + 1) + ' ред'; //+ alphabet[index - disabledCount];
            }
            return null;
        };
        ...
        var seatName = function seatName(row, column) {
            if (!row.disabled && !column.disabled) {
                var rowIndex = parseInt(rowName(row.index, row.disabled, row.disabledCount));
                var columnIndex = parseInt(columnName(column.index, column.disabled, column.disabledCount));
                let seatName = (options.map.columns * rowIndex) - options.map.columns + columnIndex;
        ...
        var setSeat = function setSeat(type) {
            if (options.map[type] && options.map[type].seats) {
                ...
                for (var i = 0; i < options.map[type].seats.length; i += 1) {
                    var index = options.map[type].seats[i] - 1;
            ...
            for (var n = 0; n < options.types.length; n += 1) {
                ...
                if ({}.hasOwnProperty.call(seatType, 'selected') && seatType.selected) {
                    ...
                    for (var l = 0; l < seatType.selected.length; l += 1) {
                        ...
                        if (!element.className.includes("blank")) {
        ...
        var createCartTotal = function createCartTotal() {
            var container = document.createElement('div');
            container.classList.add("c1");

            cartTotal = createSmallTitle('Общо: {0} {1}'.format(self.getTotal(), options.cart.currency));
        ...
        var createCart = function createCart() {
            ...
            var cartTitle = createIconedTitle(
                'Избрани билети',
                '/pics/icons/basket-icon.png'.format(options.assets.path),
                'Shopping cart icon.'
            );

            var cartTableContainer = document.createElement('div');
            cartTableContainer.classList.add('sc-cart');
            cartTableContainer.style.width = 'auto';
            cartTableContainer.style.height = 'auto';
